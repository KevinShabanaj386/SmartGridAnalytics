...
def get_config(key: str, default: Any = None, use_cache: bool = True) -> Any:
    ...
    try:
        index, data = client.kv.get(f'config/weather-producer/{key}')
...
def get_all_configs(prefix: str = 'config/weather-producer/') -> Dict[str, Any]:
    """
    Get all configuration values with a given prefix
    
    Args:
        prefix: Key prefix (default: 'config/weather-producer/')
