// Albania Consumption JavaScript
...
async function loadHistoricalData() {
    try {
        ...
        const response = await fetch(`/api/albania/consumption/historical?hours=${hours}`);
        const data = await response.json();
    ...
    if (consumption.total_consumption_mwh) {
        container.innerHTML = `
            <div class="region-consumption" style="background: rgba(0, 255, 136, 0.1); border-left: 3px solid var(--neon-green); margin-bottom: 15px;">
                <div>
                    <div class="region-name" style="font-size: 1.2rem;">Total për Shqipërinë</div>
                </div>
                <div style="text-align: right;">
