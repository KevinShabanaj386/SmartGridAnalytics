// Greece Dashboard JavaScript
let consumptionChart = null;
let priceSparklineChart = null;
let refreshInterval = null;

// Load all Greece data
async function loadGreeceDashboard() {
    try {
        ...
        console.error('Error loading Greece dashboard:', error);
        const statusEl = document.getElementById('serviceStatus');
        const statusMsg = document.getElementById('statusMessage');
...
async function loadWeatherSummary() {
    ...
    try {
        const response = await fetch('/api/greece/weather');
        if (!response.ok) {
...
async function loadPricesSummary() {
    ...
    try {
        const response = await fetch('/api/greece/prices');
        if (!response.ok) {
            const errorData = await response.json().catch(() => ({ error: `HTTP ${response.status}` }));
            throw new Error(errorData.error || `HTTP ${response.status}`);
        }
...
async function loadPricesLongTermSummary() {
    ...
    try {
        const response = await fetch('/api/greece/prices/historical');
        const data = await response.json();
