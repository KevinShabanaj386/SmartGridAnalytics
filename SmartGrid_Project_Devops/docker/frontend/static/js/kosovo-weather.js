// Kosovo Weather Data JavaScript
...
function updateTemperatureChart(weatherData) {
    ...
    const cities = weatherData.map(w => w.city_name || w.city);
    const temps = weatherData.map(w => w.temperature);
    const feelsLike = weatherData.map(w => w.feels_like || w.apparent_temperature || w.temperature);
...
function updateCitiesDetails(weatherData) {
    ...
    container.innerHTML = weatherData.map(w => `
        <div class="city-weather-item" style="margin-bottom: 15px;">
            <div>
                <strong style="font-size: 1.1rem;">${w.city_name || w.city}</strong>
                <span class="source-badge">${w.region || 'N/A'}</span>
                <div style="margin-top: 8px; font-size: 0.9rem; color: #6b7280;">
                    <div>🌡️ ${w.temperature}°C (ndjen si: ${w.feels_like || w.apparent_temperature || w.temperature}°C)</div>
                    <div>💧 Lagështia: ${w.humidity}%</div>
                    <div>🌬️ Erë: ${w.wind_speed || 0} km/h</div>
                    <div>☁️ ${w.weather_condition || 'N/A'}</div>
...
window.addEventListener('load', () => {
    loadWeatherData();
    setInterval(loadWeatherData, 60000); // Refresh every minute
});
