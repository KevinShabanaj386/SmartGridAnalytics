...
def get_config(key: str, default: Any = None, use_cache: bool = True) -> Any:
    ...
    try:
        index, data = client.kv.get(f'config/notification/{key}')
...
def get_all_configs(prefix: str = 'config/notification/') -> Dict[str, Any]:
    """
    Get all configuration values with a given prefix
    
    Args:
        prefix: Key prefix (default: 'config/notification/')
