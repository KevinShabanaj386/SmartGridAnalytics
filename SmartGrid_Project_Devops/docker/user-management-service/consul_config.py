...
def get_config(key: str, default: Any = None, use_cache: bool = True) -> Any:
    ...
    Args:
        key: Configuration key (e.g., 'postgres/host')
    ...
    try:
        index, data = client.kv.get(f'config/user-management/{key}')
...
def get_all_configs(prefix: str = 'config/user-management/') -> Dict[str, Any]:
    """
    Get all configuration values with a given prefix
    
    Args:
        prefix: Key prefix (default: 'config/user-management/')
